...
<div class="{{$attributes['class']}} mb-4">
    ...
    @elseif($attributes['type']=='textarea')
        ...
        <textarea id="{{$attributes['name']}}"
                  ...
                  class="mt-1 w-full border-gray-300 focus:border-turquoise-500 focus:ring-turquoise-500 rounded-md shadow-sm p-3 dark:border-gray-600 dark:bg-dark-eval-1
dark:text-gray-300 dark:focus:ring-offset-dark-eval-1 "
    ...
    @elseif($attributes['type'] == 'file')
        @php($existFile = isset($item[str_replace('item.','',$attributes['name'])]) && $item[str_replace('item.','',$attributes['name'])] &&  is_string($item[str_replace('item.','',$attributes['name'])]))
        <diva
        ...
        ->temporaryUrl(
        $item[str_replace('item.','',$attributes['name'])],
        now()->addMinutes(10),
        ['ResponseContentDisposition' => 'inline']
        ))
                    @else
                        @php($preview = asset(\Illuminate\Support\Facades\Storage::url($item[str_replace('item.','',$attributes['name'])])))
                    @endif
                    ...
                    <div class="flex-shrink pt-6 flex gap-2">
                        ...
                        >
                            ...
                            <span class="text-xs text-turquoise-500">
                                @svg('akar-cloud-download', 'h-5 w-5')
                            </span>
                            </x-secondary-button>
                        </a>
                        <x-secondary-button @click="show = !show" class="py-3">
                            <div class="text-xs text-turquoise-500">
                                <template x-if="!show">
                                    @svg('akar-eye-open', 'h-5 w-5')
                                </template>
                                <template x-if="show">
                                    @svg('akar-eye-slashed', 'h-5 w-5')
                                </template>
                            </div>
                        </x-secondary-button>
                    </div>
                @elseif(isset($item[str_replace('item.','',$attributes['name'])]) && $item[str_replace('item.','',$attributes['name'])] && !is_string($item[str_replace('item.','',$attributes['name'])]))
                    @php($preview = $item[str_replace('item.','',$attributes['name'])]->temporaryUrl())
                    <div class="flex-shrink pt-6 flex gap-2">
                        <a href="{{ $preview }}"
                           onclick="window.open('{{ $preview }}', 'newwindow', 'width=800,height=600'); return false;"
            ...
            @if(isset($item[str_replace('item.','',$attributes['name'])]) && $item[str_replace('item.','',$attributes['name'])] && is_string($item[str_replace('item.','',$attributes['name'])]))
                @if(env('FILESYSTEM_DISK') == 's3')
    ...
    ['ResponseContentDisposition' => 'inline']
                ...
                <div>
                    <template x-if="show">
                        <div class="mt-2">
                            <object
                                class="w-full h-auto"
                                data="{{$preview}}" type="{{$attributes['accept'] ?: 'application/pdf'}}">
            ...
            @elseif(isset($item[str_replace('item.','',$attributes['name'])]) && $item[str_replace('item.','',$attributes['name'])] && !is_string($item[str_replace('item.','',$attributes['name'])]))
                @php($preview = $item[str_replace('item.','',$attributes['name'])]->temporaryUrl(['ResponseContentDisposition' => 'inline']))
                <div>
                    <template x-if="show">
                        <div class="mt-2">
                            <object
                                class="w-full h-auto"
                                data="{{$preview}}"
                                type="{{$attributes['accept'] ?: 'application/pdf'}}">
                            </object>
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <div
                            ...
                            <div class="text-sm text-gray-600">
                                ...
                                <textarea
                                    ...
                                    class="mt-2 w-full border-gray-300 focus:border-turquoise-500 focus:ring-turquoise-500 rounded-md shadow-sm p-3 dark:border-gray-600 dark:bg-dark-eval-1 dark:text-gray-300 dark:focus:ring-offset-dark-eval-1"
                                    rows="3"
                                    placeholder="Alamat">
                                </textarea>
                            </div>
                        ...
                        <x-button
                            class="px-4 py-2 bg-turquoise-600 text-white rounded"
                            @click="show = false"
                        >
                            Close
                        </x-button>
