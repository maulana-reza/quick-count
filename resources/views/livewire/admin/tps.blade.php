    ...
    <table class="w-full whitespace-nowrap rounded dark:border-turquoise-500 table-bordered table overflow-hidden"
        ...
        @forelse($tps as $tp)
            ...
            <tr class="text-left {{ $loop->even ? 'bg-gray-100 dark:bg-dark-eval-2' : 'bg-white dark:bg-dark-eval-1' }}">
                ...
                <td class="px-3 py-2 text-sm whitespace-pre-wrap">{{ $suara['suara_sah'] ?? 0 }}</td>
                <td class="px-3 py-2 text-sm whitespace-pre-wrap">{{ $suara['suara_tidak_sah'] ?? 0 }}</td>
                <td class="px-3 py-2 text-sm whitespace-pre-wrap"><x-button class="flex-shrink" wire:click="showDetail({{ $tp->id }})">
