...
<div class="bg-white dark:bg-dark-eval-1 rounded-lg px-4 py-4 custom-scrollbar">
    <div class="flex justify-between">
        <div class="text-2xl">Formulir</div>        <x-button type="submit" wire:click="$dispatchTo('formulir-reference-child', 'showCreateForm');" class="text-blue-500">
    ...
    <div class="mt-6">
        <div class="flex justify-between">
            ...
            <div class="flex">
                ...
                <x-tall-crud-page-dropdown />    @livewire('formulir-reference-child')
            </div>
        </div>
        <table class="w-full my-4 whitespace-nowrap rounded shadow overflow-hidden" wire:loading.class.delay="opacity-50">
            <thead class="bg-turquoise-500 dark:text-white text-white font-bold">
                <tr class="">
                <td class="px-3 py-2  text-sm whitespace-pre-wrap" >No. Formulir</td>
            ...
            @forelse($results as $result)
                <tr class="text-left bg-turquoise-500 turquoise-500">
                    <td class="px-3 py-2  text-sm whitespace-pre-wrap" >{{ $result->no_formulir }}</td>
                    <td class="px-3 py-2  text-sm whitespace-pre-wrap" >{{ $result->foto }}</td>
                    <td class="px-3 py-2  text-sm whitespace-pre-wrap" >{{ $result->status_form }}</td>
                    <td class="px-3 py-2  text-sm whitespace-pre-wrap" >
                        <x-button variant="danger" type="submit" wire:click="$dispatchTo('formulir-reference-child', 'showDeleteForm', { formulir: {{ $result->id}} });" class="text-red-500">
